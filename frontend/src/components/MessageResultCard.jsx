...
import { ShieldAlert, ShieldCheck, AlertTriangle, MessageSquare, Search, Brain, Tag, ChevronRight, ChevronDown, Zap, Shield, Info } from 'lucide-react';
...
const getMsgGuidance = (score, isHighRisk, isMediumRisk) => {
  if (isHighRisk) return {
    icon: '🚨', text: 'This message is highly likely to be a scam. Do NOT click any links or share sensitive information.',
    bg: 'bg-red-500/10 border-red-500/20', textColor: 'text-red-200',
  };
  if (isMediumRisk) return {
    icon: '⚠️', text: 'This message shows some suspicious patterns. Verify directly with the official organization before taking action.',
    bg: 'bg-amber-500/10 border-amber-500/20', textColor: 'text-amber-200',
  };
  return {
    icon: '✅', text: 'This message appears safe, but always remain cautious when sharing personal or financial information.',
    bg: 'bg-emerald-500/10 border-emerald-500/20', textColor: 'text-emerald-200',
  };
};

const MessageResultCard = ({ score, threatType, reasons, intent, matchedPatterns, keywordScore, scamSignals, safeSignals }) => {
  const [detailsOpen, setDetailsOpen] = useState(false);

  const isHighRisk = score >= 71 || ['scam','phishing','malicious'].some(t => threatType?.toLowerCase().includes(t));
  ...
  if (isHighRisk) {
    ...
    Icon = ShieldAlert; headerText = 'Scam Detected';
    ...
    Icon = AlertTriangle; headerText = 'Suspicious Message';
  } else {
    ...
    Icon = ShieldCheck; headerText = 'Message is Safe';
  }
  ...
  const guidance = getMsgGuidance(score, isHighRisk, isMediumRisk);
      ...
      <div className="px-8 py-4">
        <div className="flex items-start gap-2 justify-center">
          ...
          <p className="text-[11px] text-white/30 leading-relaxed text-center">
            Analysis powered by AI and may not be 100% accurate. Always verify important messages independently.
          </p>
        </div>
      </div>
